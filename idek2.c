...
void driveMotors(int leftSpeed, int rightSpeed) {
  motor[driveFR] = normalizeSpeed(leftSpeed);
...
void driveStraightEncoders(int speed, int encoderTicks) {
  clearEncoders();
  while (driveBR < abs(encoderTicks) &&
         driveFR < abs(encoderTicks)) {
    int deviation = normalizeDeviation(driveBR -
                                       driveFR);
...
task main() {
  ...
  while (true) {
    ...
    case DRIVE_R_MODE:
      ...
      if (centerPressed) {
        driveStraightEncoders(-100, 4000);
      } else {
        motor[driveFR] = 0;
        motor[driveBR] = 0;
      }
