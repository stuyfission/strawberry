...
// Control constants.
const int CONTROL_MODE_SPEED = 20;
...
task main() {
  ...
  while (true) {
    ...
    if (controlDriveMode) {
      motor[driveFL] = CONTROL_MODE_SPEED * -(y2 / abs(y2));
      motor[driveBL] = CONTROL_MODE_SPEED * -(y2 / abs(y2));
      motor[driveFR] = CONTROL_MODE_SPEED * (y1 / abs(y1));
      motor[driveBR] = JOYSTICK_THRESHOLD * (y1 / abs(y1));
    } else {
      motor[driveFL] = y1;
      motor[driveBL] = y1;
      motor[driveFR] = -y2;
