...
/*
 * $Id: common.h 66 2011-09-09 09:41:53Z xander $
 */
...
void clearI2CError(tSensors link, ubyte address) {
  ...
  for (int i = 0; i < 5; i++) {
    sendI2CMsg(link, (char *) error_array[0], 0);
    wait1Msec(5);
  }
}
...
bool writeI2C(tSensors link, tByteArray &data, int replylen) {
  ...
  sendI2CMsg(link, (char *) data[0], replylen);
...
bool readI2C(tSensors link, tByteArray &data, int replylen) {
  ...
  readI2CReply(link, (char *) data[0], replylen);
